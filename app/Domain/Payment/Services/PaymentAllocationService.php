...
final class PaymentAllocationService
{
    ...
    public function allocateConfirmedPayment(Payment $payment): array
    {
        ...
        if (! $payment->order_id) {
            return $this->unhandledResult($payment);
        }
        ...
        return DB::transaction(function () use ($payment) {
            ...
            Order::query()->where('order_no', $orderNo)->lockForUpdate()->get(['id']);
            ...
            $idempotencyKey = 'payment:' . $lockedPayment->id . ':food-order-group:' . $orderNo;
    ...
    private function unhandledResult(Payment $payment, string $reason = 'unsupported_target'): array
    {
        return [
            'handled' => false,
            'reason' => $reason,
