...
class PaymentReconciliationService
{
    ...
    public function reconcilePendingPayments(int $limit = 50): array
    {
        $payments = Payment::query()
            ->whereIn('status', [
                'INITIATED', 'CREATED', 'PENDING', 'PROCESSING', 'AUTHORIZED',
                'SUBMITTED', 'UNKNOWN',
            ])
    ...
    protected function verifyFailedPayment(Payment $payment): array
    {
        try {
            ...
            if ($gatewayStatus->isFailed()) {
                $failureMeta = $this->failureMetaFromGatewayStatus($gatewayStatus);
                $this->updateReconciliationMeta($payment, 'FAILED_VERIFIED', $gatewayStatus, $failureMeta);
                $this->logReconciliation($payment, 'FAILED_VERIFIED', 'Échec confirmé par le provider');
    ...
    protected function checkPendingPayment(Payment $payment): array
    {
        try {
            ...
            if ($gatewayStatus->isPaid()) {
                ...
                $this->logReconciliation($fresh, 'RECONCILED', 'Paiement non résolu mis à jour vers PAID');
            ...
            if ($gatewayStatus->isFailed()) {
                ...
                $this->updateReconciliationMeta($failed, 'FAILED', $gatewayStatus, $failureMeta);
                $this->logReconciliation($failed, 'FAILED', 'Paiement échoué chez le provider');
            ...
            if ($gatewayStatus->isPending()) {
                ...
                if (in_array($current, [PaymentStatus::INITIATED, PaymentStatus::UNKNOWN], true)) {
                ...
                $this->updateReconciliationMeta($payment->fresh() ?? $payment, 'PENDING', $gatewayStatus);
            ...
            $this->updateReconciliationMeta($unknown->fresh() ?? $unknown, 'UNKNOWN', $gatewayStatus);
            $this->logReconciliation($unknown, 'UNKNOWN', 'Statut provider non déterminé');
    ...
    {
        return $this->gatewayStatusFromProviderArray($this->getProviderStatus($payment));
    }
    ...
    protected function getProviderStatus(Payment $payment): array
    {
        ...
        return [
            ...
            'message' => $gatewayStatus->failureReason ?? ($gatewayStatus->meta['message'] ?? null),
    ...
    private function gatewayStatusFromProviderArray(array $raw): GatewayStatus
    {
        ...
        return match (true) {
            ...
            in_array($status, ['PENDING', 'PROCESSING', 'AUTHORIZED', 'SUBMITTED', 'CREATED', 'INITIATED'], true)
    ...
    {
        $failureMessage = $gatewayStatus->meta['failure_message'] ?? $gatewayStatus->failureReason;
    ...
    protected function buildHistoricalFailureFallback(
        ...
        return [
            ...
            'failure_action' => 'Relancez une nouvelle requête uniquement après confirmation de l’absence de débit.',
        ];
    }
