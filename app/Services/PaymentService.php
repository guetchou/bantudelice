...
class PaymentService
{
    ...
    public function handleCallback(string $provider, array $payload): void
    {
        ...
        if ($gatewayStatus->isFailed()) {
            ...
            // Propager le statut d'échec sur la commande (business_status reste accepted_awaiting_payment,
            // le client peut réessayer le paiement — seul payment_status change).
            if ($payment->order_id) {
                ...
                try {
                    ...
                    if ($failedOrder?->user_id) {
                        NotificationService::sendToUser(
                            $failedOrder->user_id,
                            'Paiement echoue',
                            'Le paiement de votre commande #' . $failedOrder->order_no . ' a echoue. Veuillez reessayer.',
                            [
                                'key'     => 'paymentFailed',
                                'channel' => 'user',
                                'module'  => 'food',
                    ...
                    Log::warning('PaymentService: erreur notification echec paiement', [
                        'payment_id' => $payment->id,
                        'error'      => $e->getMessage(),
                    ]);
                }
