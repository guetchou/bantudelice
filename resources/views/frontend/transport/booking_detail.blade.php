...
@section('title', 'Suivi de trajet #' . $booking->booking_no . ' | Kende')
@section('description', 'Consultez le suivi de votre trajet Kende.')
...
@php
    ...
    $timeline = [
        ...
        ['key' => 'paid', 'label' => 'Paiement confirme', 'icon' => 'credit-card'],
    ];
@endphp

@php
    $paymentExperience = $paymentExperience ?? null;
    $bookingReference = $booking->uuid ?: $booking->id;
    $bookingQrTarget = route('transport.booking.show', ['id' => $bookingReference]);
@endphp
...
<style>
    ...
    .trip-hero {
        ...
        border-radius: 34px;
        color: #ffffff;
        background: linear-gradient(135deg, #FF6B00 0%, #f59e0b 60%, #facc15 100%);
    ...
    .trip-call-btn {
        ...
        justify-content: center;
        gap: 10px;
        background: linear-gradient(135deg, #059669 0%, #009B3A 100%);
        color: #ffffff;
        font-weight: 900;
...
<section class="trip-shell">
    <div class="container">
        <section class="trip-hero">
            ...
            <h1>Votre trajet Kende en temps reel.</h1>
            <p>Trajet <strong>#{{ $booking->booking_no }}</strong> · Statut actuel: <strong id="heroStatusLabel">{{ $statusLabels[$booking->status->value] ?? ucfirst($booking->status->value) }}</strong>.</p>
        </section>
        ...
        <div class="trip-layout">
            <div style="display:grid; gap:24px;">
                ...
                <section class="trip-card trip-timeline-card">
                    <h2 class="trip-card__title">Progression du trajet</h2>
                        ...
                        @foreach($timeline as $index => $step)
                            ...
                            <article class="trip-step{{ $isDone ? ' is-done' : '' }}{{ $isActive ? ' is-active' : '' }}" data-status-step="{{ $step['key'] }}">
                                ...
                                <div class="trip-step__copy">
                                    ...
                                    <small>{{ $isActive ? 'Etape actuelle' : 'En attente' }}</small>
                                </div>
                            </article>
                        @endforeach
                    </div>
            ...
            <aside class="trip-card trip-side-card">
                ...
                <section class="trip-fare-card">
                    ...
                    <div style="margin-top:18px; padding-top:18px; border-top:1px solid rgba(15,23,42,0.08); text-align:center;">
                        <small style="display:block; color:#64748b; margin-bottom:8px;">QR de reservation</small>
                        <div style="display:inline-flex; align-items:center; justify-content:center; width:160px; min-height:160px; max-width:100%; background:#fff; border-radius:18px; padding:8px; border:1px solid rgba(15,23,42,0.08);">
                            {!! QrCode::format('svg')->size(148)->margin(1)->generate($bookingQrTarget) !!}
                        </div>
                        <small style="display:block; margin-top:10px; color:#0f172a; font-weight:700;">{{ $booking->booking_no }}</small>
                        <a href="{{ $bookingQrTarget }}" style="display:inline-block; margin-top:6px;">Ouvrir la reservation</a>
                    </div>
                    <div style="margin-top:14px; display:flex; gap:10px; flex-wrap:wrap;">
                        <button type="button" class="trip-call-btn" onclick="window.print()" style="justify-content:center;">
...
<script>
    ...
    async function drawRoute(pickup, dropoff) {
        try {
            ...
            if (data.routes && data.routes[0] && data.routes[0].geometry) {
                ...
                routeLayer = L.geoJSON(data.routes[0].geometry, {
                    style: { color: '#FF6B00', weight: 5, opacity: 0.9 }
                }).addTo(map);
        ...
        routeLayer = L.geoJSON(line, {
            style: { color: '#FF6B00', weight: 4, dashArray: '10 8' }
        }).addTo(map);
        map.fitBounds(routeLayer.getBounds(), { padding: [28, 28] });
    }
    ...
    async function readBookingJson(response) {
        try {
    ...
    function bookingStatusUrl() {
        const url = new URL(`{{ url('transport/xhr/bookings') }}/${BOOKING_UUID}`, window.location.origin);
    ...
    function pollTrip() {
        ...
        .then(({ ok, data }) => {
            if (!ok || !data?.uuid || !data?.status) {
                throw new Error('Aucune confirmation exploitable n’a ete retournee pour cette reservation.');
            }
    ...
    function pollTransportPaymentStatus(paymentId, attempt = 0) {
        ...
        .then(({ ok, data }) => {
            if (!ok) {
                throw new Error(data?.message || 'Le serveur n’a pas confirme un statut 200 pour le paiement.');
            }
            ...
            if (status === 'PAID') {
                ...
                document.getElementById('heroStatusLabel').textContent = STATUS_LABELS['paid'] || 'paid';
                const bookingStatusChip = document.getElementById('bookingStatusChip');
                if (bookingStatusChip) {
                    bookingStatusChip.innerHTML = `<i class="fas fa-info-circle"></i> ${STATUS_LABELS['paid'] || 'paid'}`;
                }
                updateTimeline('paid');
    ...
    function payNow(event) {
        ...
        .then(({ ok, data }) => {
            if (!ok) {
                throw new Error(data?.message || 'Le serveur n’a pas confirme un statut 200 pour initier le paiement.');
            }
