...
<head>
    ...
    <title>@yield('title', 'Kende')</title>
    <meta name="description" content="@yield('description', 'Kende au Congo : taxi, covoiturage, location et reservations.')">
    ...
    <style>
        :root{
            --module-primary:#FF6B00;
            --module-secondary:#009B3A;
            --module-ink:#111113;
            --module-muted:#64748b;
            --module-line:rgba(0,0,0,.07);
            --module-bg:#F5F5F7;
        }
        ...
        .module-shell__brand-mark{
            ...
            height:42px;
            border-radius:14px;
            background:linear-gradient(135deg,var(--module-primary),#ff9b52);
            color:#fff;
            font-size:.9rem;
            font-weight:900;
            letter-spacing:.08em;
...
<body class="bd-transport-shell @yield('body_class')">
@php
    $transportAccountUrl = auth()->check() ? route('user.profile') : route('user.login', ['redirect' => url()->current()]);
    $transportAccountLabel = auth()->check() ? 'Mon compte' : 'Connexion';
...
<header class="module-shell__header">
    <div class="module-shell__header-inner">
        <a href="{{ route('transport.index') }}" class="module-shell__brand">
            <span class="module-shell__brand-mark" aria-hidden="true">K</span>
            <strong>Kende</strong>
        </a>
        <nav class="module-shell__nav" aria-label="Navigation transport">
            <a href="{{ route('transport.index') }}" class="{{ request()->routeIs('transport.index') ? 'is-active' : '' }}">Accueil</a>
            <a href="{{ route('transport.taxi') }}" class="{{ request()->routeIs('transport.taxi') ? 'is-active' : '' }}">Taxi</a>
            <a href="{{ route('transport.carpool') }}" class="{{ request()->routeIs('transport.carpool') ? 'is-active' : '' }}">Covoiturage</a>
            <a href="{{ route('transport.rental') }}" class="{{ request()->routeIs('transport.rental') ? 'is-active' : '' }}">Location</a>
            <a href="{{ route('transport.bus') }}" class="{{ request()->routeIs('transport.bus') ? 'is-active' : '' }}">Bus</a>
            @auth
                <a href="{{ route('transport.my_bookings') }}" class="{{ request()->routeIs('transport.my_bookings', 'transport.booking.show') ? 'is-active' : '' }}">Mes réservations</a>
            @endauth
        </nav>
        <div class="module-shell__actions">
            <a href="{{ route('help', ['brand' => 'kende']) }}" class="module-shell__button module-shell__button--ghost">Aide</a>
            <a href="{{ $transportAccountUrl }}" class="module-shell__button module-shell__button--ghost">{{ $transportAccountLabel }}</a>
            <a href="{{ route('transport.taxi') }}" class="module-shell__button module-shell__button--primary">Réserver</a>
        </div>
    </div>
</header>
@endunless
    ...
    <div class="module-shell__footer-inner">
        <div class="module-shell__footer-grid">
            <div class="module-shell__footer">
                <h4>Kende</h4>
                <p>Kende facilite vos déplacements avec une réservation claire, un suivi de course et des chauffeurs vérifiés.</p>
            </div>
            <div class="module-shell__footer">
                <h4>Transport</h4>
                <div class="module-shell__footer-links">
                    <a href="{{ route('transport.index') }}">Accueil</a>
                    <a href="{{ route('transport.taxi') }}">Taxi</a>
                    <a href="{{ route('transport.carpool') }}">Covoiturage</a>
                    <a href="{{ route('transport.rental') }}">Location</a>
                    <a href="{{ route('transport.bus') }}">Bus</a>
                    @auth
                        <a href="{{ route('transport.my_bookings') }}">Mes réservations</a>
                    @endauth
                </div>
            </div>
            <div class="module-shell__footer">
                <h4>Support</h4>
                <div class="module-shell__footer-links">
                    <a href="{{ route('help', ['brand' => 'kende']) }}">Centre d'aide</a>
