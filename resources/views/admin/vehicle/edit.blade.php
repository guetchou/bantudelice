...
@section('page_title', 'Modifier véhicule')
...
.veh-form-card__head { padding: 16px 20px; border-bottom: 1px solid #f3f4f6; }
...
.veh-licence-upload {
    ...
    text-align: center; cursor: pointer; transition: border-color .12s; background: #fafafa;
}
.veh-licence-upload:hover { border-color: #22c55e; }
.veh-licence-preview { width: 90px; height: auto; max-height: 90px; object-fit: contain; display: none; margin: 0 auto 8px; }
...
<div class="veh-form-wrap">
    ...
    <div class="veh-form-card">
        <div class="veh-form-card__head">
            <h3>Mettre à jour le véhicule</h3>
            <p>Modifiez les informations du véhicule et du livreur associé.</p>
        </div>

        <form method="post" action="{{ route('vehicle.update', $vehicel->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="veh-form-body">

                <div class="veh-field">
                    <label>Remplacer l'image du permis</label>
                    <div class="veh-licence-upload" onclick="document.getElementById('file_input').click()">
                        ...
                        <div class="veh-licence-label">
                            ...
                            <span>Cliquez</span> pour changer l'image du permis (optionnel)
                        </div>
                    </div>
