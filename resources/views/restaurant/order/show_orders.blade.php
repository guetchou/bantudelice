            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item "><a href="{{ route('restaurant.all_orders') }}">Commandes</a></li>
                ...
                <div class="col-11">
                        ...
                        <div class="col-md-4">
                            <div class="card card-primary card-outline">
                              <div class="card-body box-profile">
                                ...
                                <a href="{{ route('restaurant.profile') }}" class="btn bg-gradient-primary btn-block"><b>Restaurant</b></a>
                              </div>
                              <!-- /.card-body -->
                            </div>
                        </div>
                        <div class="col-md-4">
                            ...
                            <div class="card card-warning card-outline">
                              <div class="card-body box-profile">
                                <div class="text-center">
                                    ...
                                    <img class="rounded-circle" style="border: 2px solid #ffc107;" width="75" height="75" src="{{ !empty($order->driver->image) ? asset('images/driver_images/'.$order->driver->image) : asset('images/placeholder.png') }}" alt="Photo de profil utilisateur" onerror="this.src='{{ asset('images/placeholder.png') }}'">
                                    @else
                                    <img class="rounded-circle" style="border: 2px solid #ffc107;" width="75" height="75" src="{{asset('images/5-512.png')}}" alt="Photo de profil utilisateur">
                                    @endif
                                </div>
                                ...
                                @endphp
                                @if($driverPhoneLink)
                                    <a href="tel:{{ $driverPhoneLink }}" class="btn bg-gradient-warning btn-block"><b>Appeler le livreur</b></a>
                                @else
                                    <button type="button" class="btn bg-gradient-warning btn-block" disabled><b>Livreur</b></button>
                                @endif
                              </div>
                              <!-- /.card-body -->
                            </div>
                            ...
                            <div class="card card-warning card-outline">
                              <div class="card-body box-profile">
                                ...
                                <h3 class="profile-username text-center">Nom du livreur</h3>
                                ...
                                <button type="button" class="btn bg-gradient-warning btn-block" disabled><b>Livreur non assigné</b></button>
                              </div>
                              <!-- /.card-body -->
                            </div>
                            @endif
                        ...
                        <div class="col-md-4">
                            <div class="card card-success card-outline">
                              <div class="card-body box-profile">
                                <div class="text-center">
                                    @if($order->user->image)
                                    <img class="rounded-circle" style="border: 2px solid #009543;" width="75" height="75" src="{{ $order->user ? $order->user->avatarUrl() : asset('assets/images/user-avatar.png') }}" alt="Photo de profil utilisateur">
                                    @else
                                    <img class="rounded-circle" style="border: 2px solid #009543;" width="75" height="75" src="{{asset('images/5-512.png')}}" alt="Photo de profil utilisateur">
                                    @endif
                                </div>
                                ...
                                @endphp
                                @if($userPhoneLink)
                                    <a href="tel:{{ $userPhoneLink }}" class="btn bg-gradient-success btn-block"><b>Appeler le client</b></a>
                                @else
                                    <button type="button" class="btn bg-gradient-success btn-block" disabled><b>User</b></button>
                                @endif
                              </div>
                              <!-- /.card-body -->
                            </div>
                  ...
                  <tbody>
                    ...
                    @endforeach
                            ...
                            {{-- ── Actions commande (refus avec motif) ───────── --}}
                            @if(!in_array($order->status, ['completed', 'cancelled', 'delivered']))
                            <div class="card card-outline card-danger mt-3">
                                <div class="card-header"><h3 class="card-title"><i class="fas fa-times-circle mr-1"></i> Refuser la commande</h3></div>
                                <div class="card-body">
                                    <form method="POST" action="{{ route('restaurant.cancel_order', $order->id) }}" id="refuseOrderForm">
                                        @csrf
                                        <div class="form-group">
                                            <label class="font-weight-bold">Motif du refus <span class="text-danger">*</span></label>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea name="cancel_note" class="form-control" rows="2" placeholder="Précision pour le client…" maxlength="300"></textarea>
                                        </div>
                                        <button type="submit" class="btn btn-danger" onclick="return confirm('Confirmer le refus de cette commande ?')">
                                            <i class="fas fa-ban mr-1"></i> Refuser et notifier le client
                                        </button>
                                    </form>
                                </div>
                            </div>
                            ...
                            <div class="card card-outline card-info mt-3">
                                <div class="card-header">
                                    <h3 class="card-title"><i class="fas fa-comments mr-1"></i> Messagerie — Client & Support</h3>
                                    @if(($chatData['unread_count'] ?? 0) > 0)
                                        <span class="badge badge-danger ml-2">{{ $chatData['unread_count'] }} non lu(s)</span>
                                    @endif
                                </div>
                           ...
                           <div class="row mt-3">
                               <div class="col-md-6">
                                   <div class="small-box bg-light">
                                       <div class="inner">
                                           <p class="mb-1 text-muted">Restaurant vers client</p>
                                           <h5 id="restaurantCustomerDistance" class="mb-0">Calcul en cours</h5>
                                       </div>
                                   </div>
                               </div>
                               <div class="col-md-6">
                                   <div class="small-box bg-light">
                                       <div class="inner">
                                           <p class="mb-1 text-muted">Livreur vers client</p>
                                           <h5 id="driverCustomerDistance" class="mb-0">En attente de position</h5>
                                       </div>
                                   </div>
                               </div>
                           </div>
...
<script src="{{asset('plugins/datatables-bs4/js/dataTables.bootstrap4.js')}}"></script>
 <script>
        var pickup = {lat: @if($order->driver==null) -4.2767 @else {{$order->driver->latitude ?? -4.2767}}  @endif, lng: @if($order->driver==null) 15.2832  @else {{$order->driver->longitude ?? 15.2832}}  @endif};
        var dropoff = {lat: {{$order->d_lat}}, lng: {{$order->d_lng}}};
        var restaurantCoords = {lat: {{$order->restaurant->latitude ?? -4.2767}}, lng: {{$order->restaurant->longitude ?? 15.2832}}};
      ...
      function updateDistanceCards() {
          var restaurantDistance = document.getElementById('restaurantCustomerDistance');
          var driverDistance = document.getElementById('driverCustomerDistance');
          if (restaurantDistance) {
              restaurantDistance.textContent = formatDistance(
                  haversineKm(restaurantCoords.lat, restaurantCoords.lng, dropoff.lat, dropoff.lng)
              );
          }
          if (driverDistance) {
              driverDistance.textContent = formatDistance(
      ...
      function renderMapNotice(message) {
          ...
          if (box) {
              box.innerHTML = '<div style="padding:1rem;color:#6B7280;">' + message + '</div>';
          }
      }
