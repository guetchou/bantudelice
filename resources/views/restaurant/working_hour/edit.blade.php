...
@section('title', 'Modifier un horaire | ' . \App\Services\ConfigService::getCompanyName())
@section('topbar_title', 'Modifier un horaire')
...
<div style="max-width:600px;">
    ...
    <div style="background:var(--bd-surface);border:1px solid var(--bd-border);border-radius:var(--bd-radius);overflow:hidden;">
        <div style="padding:16px 20px;border-bottom:1px solid var(--bd-border-2);display:flex;align-items:center;justify-content:space-between;gap:12px;">
            <div>
                <div style="font-size:13px;font-weight:700;color:var(--bd-text);">Modifier un horaire</div>
                <div style="font-size:11px;color:var(--bd-text-3);margin-top:2px;">Ajustez le jour et la plage horaire</div>
            </div>
        ...
        <form method="post" action="{{ route('working_hour.update', $workingHour->id) }}">
            ...
            <div style="padding:24px 20px;display:flex;flex-direction:column;gap:18px;">
                ...
                <div>
                    <label for="day" style="display:block;font-size:12px;font-weight:600;color:var(--bd-text);margin-bottom:6px;">
                    ...
                    <select name="day" id="day" required
                        ...
                        @foreach(['monday'=>'Lundi','tuesday'=>'Mardi','wednesday'=>'Mercredi','thursday'=>'Jeudi','friday'=>'Vendredi','saturday'=>'Samedi','sunday'=>'Dimanche'] as $val => $label)
                            <option value="{{ $val }}" {{ old('day', $workingHour->Day) === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('day')
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
                    <div>
                        <label for="opening_time" style="display:block;font-size:12px;font-weight:600;color:var(--bd-text);margin-bottom:6px;">
                        ...
                        <select name="opening_time" id="opening_time" required
                            ...
                            @for($h = 0; $h < 24; $h++)
                                ...
                                <option value="{{ $val }}" {{ old('opening_time', $workingHour->opening_time) === $val ? 'selected' : '' }}>{{ str_pad($h,2,'0',STR_PAD_LEFT) }}:00</option>
                                @php $val30 = str_pad($h,2,'0',STR_PAD_LEFT).':30:00'; @endphp
                                <option value="{{ $val30 }}" {{ old('opening_time', $workingHour->opening_time) === $val30 ? 'selected' : '' }}>{{ str_pad($h,2,'0',STR_PAD_LEFT) }}:30</option>
                            @endfor
                        </select>
                        @error('opening_time')
                            <div style="font-size:11px;color:#dc2626;margin-top:4px;">{{ $message }}</div>
                    ...
                    <div>
                        <label for="closing_time" style="display:block;font-size:12px;font-weight:600;color:var(--bd-text);margin-bottom:6px;">
                        ...
                        <select name="closing_time" id="closing_time" required
                            ...
                            @for($h = 0; $h < 24; $h++)
                                @php $val = str_pad($h,2,'0',STR_PAD_LEFT).':00:00'; @endphp
                                <option value="{{ $val }}" {{ old('closing_time', $workingHour->closing_time) === $val ? 'selected' : '' }}>{{ str_pad($h,2,'0',STR_PAD_LEFT) }}:00</option>
                                @php $val30 = str_pad($h,2,'0',STR_PAD_LEFT).':30:00'; @endphp
                                <option value="{{ $val30 }}" {{ old('closing_time', $workingHour->closing_time) === $val30 ? 'selected' : '' }}>{{ str_pad($h,2,'0',STR_PAD_LEFT) }}:30</option>
                            @endfor
                        </select>
                        @error('closing_time')
                            <div style="font-size:11px;color:#dc2626;margin-top:4px;">{{ $message }}</div>
