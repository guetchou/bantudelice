...
@section('title', 'Modifier une fermeture spéciale | ' . \App\Services\ConfigService::getCompanyName())
...
<div style="max-width:600px;">
    ...
    <div style="background:var(--bd-surface);border:1px solid var(--bd-border);border-radius:var(--bd-radius);overflow:hidden;">
        <div style="padding:16px 20px;border-bottom:1px solid var(--bd-border-2);display:flex;align-items:center;justify-content:space-between;gap:12px;">
            <div>
                <div style="font-size:13px;font-weight:700;color:var(--bd-text);">Modifier la fermeture — {{ $specialClosure->label }}</div>
                <div style="font-size:11px;color:var(--bd-text-3);margin-top:2px;">Ajustez la période d'indisponibilité enregistrée</div>
            </div>
            ...
            <div style="padding:24px 20px;display:flex;flex-direction:column;gap:18px;">
                ...
                <div>
                    ...
                    <select name="label" id="label" required
                        ...
                        @foreach(['Férié'=>'Jour férié','Congé'=>'Congé / Vacances','Fermeture exceptionnelle'=>'Fermeture exceptionnelle','Travaux'=>'Travaux / Rénovation','Inventaire'=>'Inventaire','Événement privé'=>'Événement privé / Réservé'] as $val => $lbl)
                            <option value="{{ $val }}" {{ old('label', $specialClosure->label) === $val ? 'selected' : '' }}>{{ $lbl }}</option>
                        @endforeach
                    </select>
                    @error('label')
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
                    <div>
                        ...
                        <input required type="date" name="starts_on" id="starts_on"
                               value="{{ old('starts_on', optional($specialClosure->starts_on)->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input required type="date" name="ends_on" id="ends_on"
                               value="{{ old('ends_on', optional($specialClosure->ends_on)->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              ...
                              onblur="this.style.borderColor='var(--bd-border)';">{{ old('notes', $specialClosure->notes) }}</textarea>
                </div>
