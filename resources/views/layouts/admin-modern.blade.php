...
<div class="adm-topbar-right"><span class="adm-workspace-pill"><i class="fas fa-circle" style="font-size:.42rem"></i>{{ $workspaceConfig['label']??ucfirst($activeWorkspace) }}</span><button type="button" class="adm-topbar-avatar" title="Mon profil" onclick="bdDrawerOpen()">{{ strtoupper(substr($adminUser->name??'A',0,1)) }}</button></div>
</header>
...
(function(){try{if(window.innerWidth>1024&&localStorage.getItem('adm-sidebar-collapsed')==='1')document.body.classList.add('sidebar-collapsed')}catch(e){}window.addEventListener('resize',function(){if(window.innerWidth>1024)admCloseSidebar()})})();
</script>
@yield('scripts')
@yield('script')
@stack('scripts')
